...
class MP2:
    ...
    def get_energy_df(self): 
        ...
        for i in range(nocc):
            ...
            for j in range(i,nocc):
                e_j = e[j]
                gmo_df_ab = np.einsum('aP, bP ->ab', b_iaP[i],b_iaP[j])
                for a in range(nocc,ntot):
                    for b in range(a,ntot):
                        E_df += ((gmo_df_ab[a,b]-gmo_df_ab[b,a])**2)/(e_i + e_j-e[a]-e[b])
        t3 = time.time()
...
def int_trans_df_alt(b_pqP, C):
    a = np.einsum('pqP,pi -> iqP',b_pqP,C)
    b = np.einsum('iqP,qa -> iaP',a,C)
    return np.einsum('iaP,jbP->iajb',b,b)
